...
without a licence check. `llms.txt` is the machine index and this file is
`AGENTS.md`; `LLM.md` and `CLAUDE.md` are links to it, that way round because a
symlink fetched over `raw.githubusercontent.com` serves its target's *path*, not
its contents — so the name a bot reaches for has to be the real file.
